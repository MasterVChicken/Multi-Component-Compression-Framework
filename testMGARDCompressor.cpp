...
#include "ProgressiveCompressorGPUMGARD.hpp"
...
void testProgressiveComp(const std::string &name,
  ...
  for (int i = 0; i < prog.getNumComponents(); ++i) {
    ...
    std::cout << "Component " << i + 1 << ": tol = " << relTolList[i]
...
template <typename T> int run_test(int argc, char *argv[]) {
  ...
  GPUMGARDCompressor<T> mgard_gpu;
  
  compressors.push_back(std::make_pair("MGARD_GPU", &mgard_gpu));
